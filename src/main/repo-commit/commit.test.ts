...
describe('commitRepo', () => {
  ...
  it('rejects when stage="paths" but no paths are given', async () => {
    ...
    await expect(
      commitRepo(SAFE_ROOTS, repo, { message: 'x', stage: 'paths', dry_run: true, allow_empty: false })
  ...
  it('rejects an empty message', async () => {
    ...
    await expect(
      commitRepo(SAFE_ROOTS, repo, { message: '', stage: 'all_tracked', dry_run: true, allow_empty: false })
  ...
  it('refuses to create an empty commit by default (allow_empty=false)', async () => {
    ...
    await expect(
      commitRepo(SAFE_ROOTS, repo, { message: 'empty', stage: 'none', dry_run: false, allow_empty: false })
