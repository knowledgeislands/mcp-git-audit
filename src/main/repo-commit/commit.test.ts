...
describe('commitRepo', () => {
  it('stages all tracked modifications and creates a real commit when dry_run=false', async () => {
    ...
    const result = await commitRepo(SAFE_ROOTS, repo, { message: 'update readme', stage: 'all_tracked', dry_run: false, allow_empty: false })
  ...
  it('stage="paths" only stages the listed paths', async () => {
    ...
    const result = await commitRepo(SAFE_ROOTS, repo, { message: 'add a only', stage: 'paths', paths: ['a.txt'], dry_run: false, allow_empty: false })
  ...
  it('rejects when paths are given for a non-paths stage mode', async () => {
    ...
    await expect(commitRepo(SAFE_ROOTS, repo, { message: 'x', stage: 'all_tracked', paths: ['README.md'], dry_run: true, allow_empty: false })).rejects.toThrow(/only allowed when stage="paths"/)
  })

  it('rejects relative paths that escape or look like options', async () => {
    const repo = await makeRepo('reject-paths')
    await expect(commitRepo(SAFE_ROOTS, repo, { message: 'x', stage: 'paths', paths: ['../etc/passwd'], dry_run: true, allow_empty: false })).rejects.toThrow(/invalid path/)
    await expect(commitRepo(SAFE_ROOTS, repo, { message: 'x', stage: 'paths', paths: ['-rf'], dry_run: true, allow_empty: false })).rejects.toThrow(/invalid path/)
    await expect(commitRepo(SAFE_ROOTS, repo, { message: 'x', stage: 'paths', paths: ['/abs/path'], dry_run: true, allow_empty: false })).rejects.toThrow(/invalid path/)
    await expect(commitRepo(SAFE_ROOTS, repo, { message: 'x', stage: 'paths', paths: ['foo/../bar'], dry_run: true, allow_empty: false })).rejects.toThrow(/invalid path/)
  })
  ...
  it('wraps an add-stage failure with a "git add failed:" prefix', async () => {
    ...
    await expect(commitRepo(SAFE_ROOTS, repo, { message: 'x', stage: 'paths', paths: ['no-such-file.txt'], dry_run: false, allow_empty: false })).rejects.toThrow(/git add failed:/)
  })
})
