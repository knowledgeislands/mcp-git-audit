...
describe('listRemotes', () => {
  it('returns sorted entries with fetch/push URLs and an ISO timestamp', async () => {
    ...
    const result = await listRemotes(repo)
  ...
  it('rejects abs_path outside the safe roots', async () => {
    await expect(listRemotes('/etc')).rejects.toThrow(/not inside any configured safe_root/)
  })
})
...
describe('setRemoteUrl', () => {
  it('previews on dry_run=true without changing the URL; before is populated, after is omitted', async () => {
    ...
    const result = await setRemoteUrl(repo, { remote: 'origin', url: 'https://example.com/new.git', push: false, dry_run: true })
  ...
  it('changes the fetch URL on dry_run=false; running twice is idempotent', async () => {
    ...
    const first = await setRemoteUrl(repo, { remote: 'origin', url: 'https://example.com/new.git', push: false, dry_run: false })
    expect(first.dry_run).toBe(false)
    expect(first.after?.fetch_url).toBe('https://example.com/new.git')
    const second = await setRemoteUrl(repo, { remote: 'origin', url: 'https://example.com/new.git', push: false, dry_run: false })
  ...
  it('changes only the push URL when push=true', async () => {
    ...
    const result = await setRemoteUrl(repo, { remote: 'origin', url: 'https://example.com/push.git', push: true, dry_run: false })
  ...
  it('rejects when the remote does not exist', async () => {
    ...
    await expect(setRemoteUrl(repo, { remote: 'nope', url: 'https://example.com/bar.git', push: false, dry_run: false })).rejects.toThrow(/does not exist/)
  })
})
...
describe('addRemote', () => {
  it('previews on dry_run=true', async () => {
    ...
    const result = await addRemote(repo, { remote: 'fork', url: 'https://example.com/fork.git', dry_run: true })
    expect(result.dry_run).toBe(true)
    expect(result.after).toBeUndefined()
    const remotes = await listRemotes(repo)
  ...
  it('creates a new remote on dry_run=false; second call rejects', async () => {
    ...
    const result = await addRemote(repo, { remote: 'fork', url: 'https://example.com/fork.git', dry_run: false })
    expect(result.after?.fetch_url).toBe('https://example.com/fork.git')
    await expect(addRemote(repo, { remote: 'fork', url: 'https://example.com/fork2.git', dry_run: false })).rejects.toThrow(/already exists/)
  })
})
...
describe('removeRemote', () => {
  it('previews on dry_run=true without removing', async () => {
    ...
    const result = await removeRemote(repo, { remote: 'origin', dry_run: true })
    expect(result.dry_run).toBe(true)
  ...
  it('removes the remote on dry_run=false; second call rejects', async () => {
    ...
    const result = await removeRemote(repo, { remote: 'origin', dry_run: false })
    expect(result.dry_run).toBe(false)
    expect(result.before?.fetch_url).toBe('https://example.com/origin.git')
    const remotes = await listRemotes(repo)
    expect(remotes.remotes).toEqual([])
    await expect(removeRemote(repo, { remote: 'origin', dry_run: false })).rejects.toThrow(/does not exist/)
  })
})
