...
describe('commitRepo', () => {
  it('stages all tracked modifications and creates a real commit when dry_run=false', async () => {
    ...
    const result = await commitRepo(repo, { message: 'update readme', stage: 'all_tracked', dry_run: false, allow_empty: false })
    expect(result.dry_run).toBe(false)
    expect(result.stage).toBe('all_tracked')
    expect(result.staged_paths).toContain('README.md')
    expect(result.command).toEqual(['git', 'commit', '-m', 'update readme'])
  ...
  it('previews via --dry-run when dry_run=true and reports null SHA', async () => {
    ...
    const result = await commitRepo(repo, { message: 'preview', stage: 'all_tracked', dry_run: true, allow_empty: false })
    expect(result.dry_run).toBe(true)
    expect(result.sha).toBeNull()
    expect(result.command).toEqual(['git', 'commit', '--dry-run', '-m', 'preview'])
  ...
  it('stage="all" pulls in untracked files', async () => {
    ...
    const result = await commitRepo(repo, { message: 'add new', stage: 'all', dry_run: false, allow_empty: false })
  ...
  it('stage="paths" only stages the listed paths', async () => {
    ...
    const result = await commitRepo(repo, { message: 'add a only', stage: 'paths', paths: ['a.txt'], dry_run: false, allow_empty: false })
  ...
  it('stage="none" commits the existing index without staging anything', async () => {
    ...
    const result = await commitRepo(repo, { message: 'commit pre-staged', stage: 'none', dry_run: false, allow_empty: false })
    expect(result.staged_paths).toEqual(['c.txt'])
    expect(result.command).toEqual(['git', 'commit', '-m', 'commit pre-staged'])
  })

  it('rejects when stage="paths" but no paths are given', async () => {
    const repo = await makeRepo('paths-required')
    await expect(commitRepo(repo, { message: 'x', stage: 'paths', dry_run: true, allow_empty: false })).rejects.toThrow(/paths is required/)
  })

  it('rejects when paths are given for a non-paths stage mode', async () => {
    const repo = await makeRepo('paths-mismatch')
    await expect(commitRepo(repo, { message: 'x', stage: 'all_tracked', paths: ['README.md'], dry_run: true, allow_empty: false })).rejects.toThrow(/only allowed when stage="paths"/)
  })

  it('rejects relative paths that escape or look like options', async () => {
    const repo = await makeRepo('reject-paths')
    await expect(commitRepo(repo, { message: 'x', stage: 'paths', paths: ['../etc/passwd'], dry_run: true, allow_empty: false })).rejects.toThrow(/invalid path/)
    await expect(commitRepo(repo, { message: 'x', stage: 'paths', paths: ['-rf'], dry_run: true, allow_empty: false })).rejects.toThrow(/invalid path/)
    await expect(commitRepo(repo, { message: 'x', stage: 'paths', paths: ['/abs/path'], dry_run: true, allow_empty: false })).rejects.toThrow(/invalid path/)
    await expect(commitRepo(repo, { message: 'x', stage: 'paths', paths: ['foo/../bar'], dry_run: true, allow_empty: false })).rejects.toThrow(/invalid path/)
  })

  it('rejects an empty message', async () => {
    const repo = await makeRepo('empty-msg')
    await expect(commitRepo(repo, { message: '', stage: 'all_tracked', dry_run: true, allow_empty: false })).rejects.toThrow(/must not be empty/)
  })

  it('rejects a multi-line message', async () => {
    const repo = await makeRepo('multiline-msg')
    await expect(commitRepo(repo, { message: 'line1\nline2', stage: 'all_tracked', dry_run: true, allow_empty: false })).rejects.toThrow(/single line/)
  })

  it('refuses to create an empty commit by default (allow_empty=false)', async () => {
    const repo = await makeRepo('empty-default')
    await expect(commitRepo(repo, { message: 'empty', stage: 'none', dry_run: false, allow_empty: false })).rejects.toThrow(/git commit failed:/)
  })

  it('creates an empty commit when allow_empty=true', async () => {
    const repo = await makeRepo('empty-allow')
    const result = await commitRepo(repo, { message: 'empty', stage: 'none', dry_run: false, allow_empty: true })
    expect(result.command).toEqual(['git', 'commit', '--allow-empty', '-m', 'empty'])
    expect(result.sha).not.toBeNull()
  })
  ...
  it('wraps an add-stage failure with a "git add failed:" prefix', async () => {
    ...
    await expect(commitRepo(repo, { message: 'x', stage: 'paths', paths: ['no-such-file.txt'], dry_run: false, allow_empty: false })).rejects.toThrow(/git add failed:/)
  })
})
