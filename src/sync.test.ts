...
describe('fetchRepo', () => {
  it('fetches from the configured remote and emits the executed argv', async () => {
    ...
    const result = await fetchRepo(repo, { remote: 'origin', prune: false, tags: false, all_remotes: false, dry_run: false })
  ...
  it('passes through --prune, --tags, --dry-run, and --all', async () => {
    ...
    const result = await fetchRepo(repo, { remote: 'origin', prune: true, tags: true, all_remotes: true, dry_run: true })
  ...
  it('wraps git failures with a "git fetch failed:" prefix', async () => {
    ...
    await expect(fetchRepo(repo, { remote: 'no-such-remote', prune: false, tags: false, all_remotes: false, dry_run: false })).rejects.toThrow(/git fetch failed:/)
  })
})
...
describe('pullRepo', () => {
  it('rejects ff_only=true with rebase=true', async () => {
    ...
    await expect(pullRepo(repo, { remote: 'origin', rebase: true, ff_only: true, autostash: false, dry_run: false })).rejects.toThrow(/mutually exclusive/)
  })
  ...
  it('rejects detached HEAD without an explicit branch', async () => {
    ...
    await expect(pullRepo(repo, { remote: 'origin', rebase: false, ff_only: true, autostash: false, dry_run: false })).rejects.toThrow(/detached HEAD/)
  })
  ...
  it('approximates dry_run via git fetch --dry-run and reports the executed argv', async () => {
    ...
    const result = await pullRepo(repo, { remote: 'origin', rebase: false, ff_only: true, autostash: false, dry_run: true })
  ...
  it('wraps a dry_run fetch failure with the dry-run prefix', async () => {
    ...
    await expect(pullRepo(repo, { remote: 'no-such', rebase: false, ff_only: true, autostash: false, dry_run: true })).rejects.toThrow(/pull dry-run/)
  })
  ...
  it('runs a real fast-forward pull with --ff-only and brings the working tree up to date', async () => {
    ...
    const result = await pullRepo(repo, { remote: 'origin', rebase: false, ff_only: true, autostash: false, dry_run: false })
    expect(result.dry_run).toBe(false)
  ...
  it('runs a rebase pull when rebase=true and ff_only=false (with autostash)', async () => {
    ...
    const result = await pullRepo(repo, { remote: 'origin', branch: 'main', rebase: true, ff_only: false, autostash: true, dry_run: false })
  ...
  it('wraps a real-pull failure with a "git pull failed:" prefix', async () => {
    ...
    await expect(pullRepo(repo, { remote: 'no-such', rebase: false, ff_only: true, autostash: false, dry_run: false })).rejects.toThrow(/git pull failed:/)
  })
})
...
describe('pushRepo', () => {
  it('rejects detached HEAD without an explicit branch', async () => {
    ...
    const { stdout } = await git(repo, 'rev-parse', 'HEAD~1')
    await git(repo, 'checkout', '-q', stdout.trim())
    await expect(pushRepo(repo, { remote: 'origin', force_mode: 'none', set_upstream: false, tags: false, delete: false, dry_run: false })).rejects.toThrow(/detached HEAD/)
  })
  ...
  it('default push with dry_run=true reports the executed argv and is non-destructive', async () => {
    ...
    const result = await pushRepo(repo, { remote: 'origin', force_mode: 'none', set_upstream: false, tags: false, delete: false, dry_run: true })
    expect(result.dry_run).toBe(true)
  ...
  it('builds --force-with-lease / --set-upstream / --tags into the argv', async () => {
    ...
    const result = await pushRepo(repo, { remote: 'origin', force_mode: 'with_lease', set_upstream: true, tags: true, delete: false, dry_run: true })
  ...
  it("builds --force when force_mode='force' (no --force-with-lease)", async () => {
    ...
    const result = await pushRepo(repo, { remote: 'origin', force_mode: 'force', set_upstream: false, tags: false, delete: false, dry_run: true })
  ...
  it('runs a real push when dry_run=false and updates the bare upstream', async () => {
    ...
    const result = await pushRepo(repo, { remote: 'origin', force_mode: 'none', set_upstream: false, tags: false, delete: false, dry_run: false })
  ...
  it('deletes a remote branch with --delete', async () => {
    ...
    const result = await pushRepo(repo, { remote: 'origin', branch: 'feature', force_mode: 'none', set_upstream: false, tags: false, delete: true, dry_run: false })
  ...
  it('wraps git failures with a "git push failed:" prefix', async () => {
    ...
    await expect(pushRepo(repo, { remote: 'no-such', force_mode: 'none', set_upstream: false, tags: false, delete: false, dry_run: false })).rejects.toThrow(/git push failed:/)
  })
})
