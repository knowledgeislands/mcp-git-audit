...
const setUrlInput = z
  .object({
    ...
    dry_run: z
      ...
      .default(true)
      .describe(
        'When true (default), no git mutation is performed; the call returns the current remote entry as `before`.'
      )
  })
  .strict()
...
const listRemotesOutput = z.object({
  abs_path: z.string(),
  fetched_at: z.string(),
  remotes: z.array(remoteEntrySchema)
})
